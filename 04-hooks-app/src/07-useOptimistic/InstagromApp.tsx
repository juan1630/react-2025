...
export const InstagromApp = () => {
  ...
  const [optimisticCommnets, addOptimisticComment] = useOptimistic(
    comments,
  ...
  const handleAddComment = async (formData: FormData) => {
    ...
    startTransition(async () => {
      ...
      // setComments((prev) => [
      //     ...prev,
      //     { id: new Date().getTime(), text: messagePost },
      // ]);

      //codigo por si falla la actualización optimista
  ...
  return (
    <div className="bg-slate-700 h-screen flex flex-col items-center justify-center">
      ...
      <ul className="flex flex-col items-start justify-center bg-gray-300 w-[500px] p-4">
        {optimisticCommnets.map((comment) => (
